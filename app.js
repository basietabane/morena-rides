...
function setDestination(lat,lng){
  ...
  if(userPos){
    ...
    document.getElementById('destination').dataset.km=
      km.toFixed(1)
  }
}
...
function useMyLocation(){
  if(!navigator.geolocation)
    return alert('GPS is not available.');
  ...
  navigator.geolocation.getCurrentPosition(p=>{
    ...
    if(pickMarker)
      pickMarker.setLatLng([userPos.lat,userPos.lng]);
    else
      pickMarker=L.marker([userPos.lat,userPos.lng]).addTo(map);
...
async function requestRide(){
  ...
  if(!dest)
    return alert('Enter or select a destination first.');

  let km=
    Number(document.getElementById('destination').dataset.km||5);
  ...
  if(sb&&session){
    const {data,error}=await sb.from('rides').insert({
      ...
      destination_lat:
        destMarker?.getLatLng().lat||null,
      destination_lng:
        destMarker?.getLatLng().lng||null,
...
function localRideToUi(r){
  return r?{
    ...
    driver_id:r.driver_id||r.driverId||null,
...
async function renderCurrent(){
  ...
  if(!r){
    el.innerHTML=
      '<div class="card"><p>No ride requested yet.</p><button onclick="show(\'home\')">Book a ride</button></div>';
...
async function toggleDriver(){
  ...
  if(driverOnline){
    const r=
      localState.rides.find(x=>x.status==='requested');
...
async function acceptRide(rid){
  ...
  const r=
    localState.rides.find(x=>x.id===rid);

  if(r){
    r.status='accepted';
    r.driverId='demo-driver';
    r.driverName=
      localState.driver?.name||
      'Morena Driver';
...
async function advanceById(rid,status){
  if(sb&&session){
    ...
    const {error}=await sb.from('rides')
      .update(patch)
      .eq('id',rid);
...
async function renderDriver(){
  ...
  if(sb&&session){
    ...
    jobs=
      localState.rides.filter(r=>
        ['requested','accepted','arrived','in_progress']
        .includes(r.status)
      )
  }

  document.getElementById('driverJobs').innerHTML=
    jobs.length?
    jobs.map(r=>{
      const idv=r.id;
      const st=r.status;
      ...
      return `<div class="job">
        ...
        ${st==='requested'?
          `<button onclick="acceptRide('${idv}')">Accept ride</button>`:
          ''}

        ${st==='accepted'?
          `<button onclick="advanceById('${idv}','arrived')">I have arrived</button>`:
          ''}

        ${st==='arrived'?
          `<button onclick="advanceById('${idv}','in_progress')">Start trip</button>`:
          ''}

        ${st==='in_progress'?
          `<button onclick="advanceById('${idv}','completed')">Complete trip</button>`:
          ''}
      </div>`
    }).join(''):
...
async function renderAdmin(){
  ...
  document.getElementById('adminRides').innerHTML=
    rides.length?
    rides.map(r=>`
      <div class="job">
...
async function renderAll(){
  ...
  document.getElementById('backendStatus').textContent=
    sb?
      (
        session?
        'Connected • '+(profile?.role||'account'):
        'Supabase configured • sign in'
      ):
...
async function signUp(){
  if(!sb)
    return alert('Add Supabase settings first.');
...
async function boot(){
  const savedRate=
    localStorage.getItem('morena_rate');
